#!/usr/bin/env python3
"""
Vocal Extraction Tool
Extracts vocals from audio files using Demucs (audio source separation).
"""
...
def extract_vocals(input_file, output_file=None, model="htdemucs", keep_temp=False):
    """
    ...
    Args:
        ...
        output_file (str, optional): Path to the output file.
                                    If None, uses input filename with '_vocals' suffix
    ...
    if output_file is None:
        output_file = input_path.parent / f"{input_path.stem}_vocals{input_path.suffix}"
    ...
    try:
        ...
        # Demucs outputs to: temp_dir/model_name/filename/vocals.wav
        separated_dir = temp_dir / model / input_path.stem
        vocals_file = separated_dir / "vocals.wav"
        
        if not vocals_file.exists():
            print(f"Error: Expected output file not found at {vocals_file}")
            return False
        ...
        if not keep_temp:
            ...
            print(f"  - vocals.wav (isolated vocals)")
            print(f"  - drums.wav (isolated drums)")
            print(f"  - bass.wav (isolated bass)")
            print(f"  - other.wav (other instruments)")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python extract_vocals.py <input_file> [output_file] [options]")
        print("\nExamples:")
        print("  python extract_vocals.py audio.m4a")
        print("  python extract_vocals.py audio.m4a vocals.m4a")
        print("  python extract_vocals.py audio.m4a --keep-temp")
        print("\nOptions:")
        print("  --keep-temp    Keep all separated stems (vocals, drums, bass, other)")
    ...
    success = extract_vocals(input_file, output_file, model, keep_temp)
